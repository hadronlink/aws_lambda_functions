...
def get_profile_details(role_id):
    ...
    if role_id.startswith('HOMEOWNER#'):
        print(f"[DEBUG] Retrieving homeowner profile from DynamoDB roles table using GSI for role_id: {role_id}")
        try:
            ...
            response = table.query(
                ...
                # Assuming these fields exist in your roles table for homeowners
            ...
            if items:
                ...
                print(f"[WARNING] Homeowner profile not found in roles GSI for role_id: {role_id}")
        except Exception as e:
            print(f"[ERROR] Error retrieving homeowner profile from DynamoDB for {role_id}: {str(e)}")
            profile_data = None

    elif role_id.startswith('PROPMNG#'):
        print(f"[DEBUG] Retrieving property manager profile from DynamoDB roles table using GSI for role_id: {role_id}")
        try:
            response = table.query(
            ...
            if items:
                ...
                print(f"[WARNING] Property manager profile not found in roles GSI for role_id: {role_id}")
        except Exception as e:
...
def handle_request(event, payload):
    ...
    try:
        ...
        print(f"[ERROR] Unexpected error in handle_request: {e}")
        traceback.print_exc()
