...
def lambda_handler(event, context):
    ...
    try:
        if operation == 'GET' and event.get('queryStringParameters'):
            payload = event['queryStringParameters']
        elif operation == 'DELETE' and event.get('body'):
            body = json.loads(event['body'])
            payload = body
        elif operation == 'PUT' and event.get('body'):  # Add this condition
...
def create_item(payload):
    ...
    try:
        ...
        # =============== Store the quote in the quotes table ===========
